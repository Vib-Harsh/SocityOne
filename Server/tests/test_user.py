...
def test_create_user_duplicate_email(client, auth_headers):
    ...
    response2 = client.post("/api/v1/users/", json=payload, headers=auth_headers)
...
def test_get_user_by_id_success(client, auth_headers):
    ...
    create_res = client.post("/api/v1/users/", json=payload, headers=auth_headers)
...
def test_list_users_search_and_sort(client, auth_headers):
    role_id = create_mock_role()
    
    # Create two users
    client.post("/api/v1/users/", json={"name": "Alice", "email": "alice@example.com", "password": "pass", "role_id": role_id}, headers=auth_headers)
    client.post("/api/v1/users/", json={"name": "Bob", "email": "bob@example.com", "password": "pass", "role_id": role_id}, headers=auth_headers)
    ...
    # List sorted
    response_sort = client.get("/api/v1/users/?sort_by=name&sort_order=DESC", headers=auth_headers)
...
def test_update_user_success(client, auth_headers):
    ...
    # Create user
    create_res = client.post("/api/v1/users/", json={"name": "Charlie", "email": "charlie@example.com", "password": "pass", "role_id": role_id}, headers=auth_headers)
    user_id = create_res.json()["id"]
    ...
    response = client.put(f"/api/v1/users/{user_id}", json=update_payload, headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
...
def test_update_user_invalid_role(client, auth_headers):
    role_id = create_mock_role()
    
    create_res = client.post("/api/v1/users/", json={"name": "Charlie", "email": "charlie@example.com", "password": "pass", "role_id": role_id}, headers=auth_headers)
    user_id = create_res.json()["id"]

    response = client.put(f"/api/v1/users/{user_id}", json={"role_id": 9999}, headers=auth_headers)
    assert response.status_code == 404
...
def test_update_user_duplicate_email(client, auth_headers):
    ...
    client.post("/api/v1/users/", json={"name": "User One", "email": "one@example.com", "password": "pass", "role_id": role_id}, headers=auth_headers)
    create_res2 = client.post("/api/v1/users/", json={"name": "User Two", "email": "two@example.com", "password": "pass", "role_id": role_id}, headers=auth_headers)
    user2_id = create_res2.json()["id"]

    # Update user 2 to use user 1's email
    response = client.put(f"/api/v1/users/{user2_id}", json={"email": "one@example.com"}, headers=auth_headers)
    assert response.status_code == 400
...
def test_delete_user_success(client, auth_headers):
    ...
    # Create user
    create_res = client.post("/api/v1/users/", json={"name": "Dave", "email": "dave@example.com", "password": "pass", "role_id": role_id}, headers=auth_headers)
    user_id = create_res.json()["id"]
    ...
    list_res = client.get("/api/v1/users/", headers=auth_headers)
